...
def addAsset(id, name, description, isActuator, assetType, style = "Undefined"):
    ...
    headers = {"Content-type": "application/json", "Auth-ClientKey": ClientKey, "Auth-ClientId": ClientId}#"Type": "intel-edison", "IconKey": "device-intel-edison"}
...
def createDevice(name, description, activityEnabled = False):
    ...
    headers = {"Content-type": "application/json", "Auth-ClientKey": ClientKey, "Auth-ClientId": ClientId,"type":"rpi"}
    url = "/Device"
    ...
    print("its jsonnnn:"+jsonStr)
    if response.status == 201:
        d = json.loads(jsonStr)
        DeviceId = d["id"]
#	print("yaaa DeviceID: " +d["id"])
    return [d["id"],d["name"]]
